...
namespace CrossSell_App.Manager
{
    ...
    {
        private ObjectivesService repo = new ObjectivesService();
        ...
        {
            var data= repo.getObjectivebyCompany(companyid);
            var dataToReturn = data.Select(p => new ObjectiveTO()
            {
                Answer = p.Answer,
                Comments = p.Comments,
